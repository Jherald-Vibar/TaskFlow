...
<div class="w-full mx-auto mt-10 px-4">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        ...
        <div class="bg-white dark:bg-gray-800 shadow-lg rounded-xl p-6">
            <h3 class="text-m font-bold text-gray-800 dark:text-white mb-4 flex items-center gap-2">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-red-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 11l3 3L22 4" />
                </svg>
                Missing Tasks for Today
            </h3>
            <div class="space-y-4">
                @foreach ($missingTasks ?? [] as $task)
                    <div class="task-card p-4 bg-gray-50 dark:bg-gray-700 rounded-lg shadow-sm hover:shadow-md transition">
                        ...
                        <p class="text-sm text-gray-500 dark:text-gray-400">Status: Missing</p>
                    </div>
                @endforeach
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 shadow-lg rounded-xl p-6">
            <h3 class="text-m font-bold text-gray-800 dark:text-white mb-4 flex items-center gap-2">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-yellow-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4V12L16 16" />
                </svg>
                Pending Tasks
            </h3>
            <div class="space-y-4">
                @foreach ($groupedTasks['Pending'] ?? [] as $task)
                    <div class="task-card p-4 bg-gray-50 dark:bg-gray-700 rounded-lg shadow-sm hover:shadow-md transition">
                        <h4 class="text-lg font-semibold text-gray-800 dark:text-white">{{ $task->task_name }}</h4>
                        <p class="text-sm text-gray-600 dark:text-gray-400">Due: {{ \Carbon\Carbon::parse($task->due_time)->format('h:i A') }}</p>
                        <p class="text-sm text-gray-500 dark:text-gray-400">Status: {{ $task->progress->status }}</p>
                    </div>
                @endforeach
            </div>
        </div>
